...
function findGrade (arrName,arrMark){
    ...
    }   return console.log(arrNameAndGrade);
}; 
...
*/
/*
function sortArr(arrMixed){
    arrOrder = [];
    for(var i = 0; i < arrMixed.length; i++){
        if (arrMixed[i] < arrMixed[i+1]){
            arrOrder[arrOrder.length] = arrMixed[i];
        }else{
            arrOrder[arrOrder.length] = arrMixed[i+1];
        }
        }return arrOrder;
    };
